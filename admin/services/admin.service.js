  ...
  return new Promise(async (resolve, reject) => {
    ...
    return await Admin.findOne({
      ...
      .then(user => {
        ...
        if(user.deleted)
        {
          resolve({ success: false, message: "Account blocked!. Contact to your administrator" });
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    await Admin.findById(id)
      .then(user => {
        ...
        if (user.username==username&&user.salt&&user.validPassword(oldPassword))
         {
           user.setPassword(password);
           user.save();
...
async function getAll() {
  return new Promise(async (resolve, reject) => {
    await Admin.find({deleted:false}).select("-salt -hash -securityCode -token")
      .then(users => {
        if (!users)
         resolve({ success: false, message: "Users not found" });
         else
