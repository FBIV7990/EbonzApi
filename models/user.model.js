...
const UserSchema = mongoose.Schema(
  {
    ...
    login_types: [ { _id: false,
                login_type:{type:String,uppercase:true,enum: ['FACEBOOK', 'GOOGLE','APPLE', 'EMAIL','MOBILE']},
                status:Boolean,
                access_token:String
                }],
    account:{
                 username: { type: String,lowercase:true, unique: true,trim:true, required:[ true,"can't be blank"] },                
...
{
  var hash= crypto.pbkdf2Sync(password, this.account.salt, 10000, 512, 'sha512').toString('hex');
