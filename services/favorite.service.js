...
async function add(req,res)
{
  try{
    const schema = Joi.object().keys({
      ...
      if(user&&post)
      {
        return await Favorite.updateOne({_id:userId},{$addToSet: {posts: postId}}).then(async res=>{ 
...
async function addSubCategory(req,res)
{
  try{
    const schema = Joi.object().keys({
      ...
      if(user)
      {
        return await Favorite.updateOne({_id:userId},{$addToSet: {subcategories: subcategoryId}}).then(async res=>{   
            console.log(res);    
        if(res.n==0)
        {
            const Fav=new Favorite();
...
async function removeSubCategory(req,res)
{
  try{
    const schema = Joi.object().keys({
      ...
      const {userId,subcategoryId}=favParam;
      return Favorite.updateOne({_id:userId},{ $pull: { subcategories:subcategoryId }}).then(async res=>{
...
async function remove(req)
{
    try{
        const schema = Joi.object().keys({
          ...
          const {userId,postId}=favParam;
         return Favorite.updateOne({_id:userId},{ $pull: { posts:postId }}).then(async res=>{
            if(res.n==1&&res.nModified==1)
            {
            await Post.updateOne({_id:postId}, { $inc: { 'likes': -1 }});
            }
    ...
    if (params.id)
     {
      ...
      {     
       const favorites = await Favorite.findById(params.id)
         ...
         return {success:true,subcategories:favorites && favorites.subcategories,posts:favorites && favorites.posts}     
       }
     else if (params.subcategories) {
        
       
        const favorites = await Favorite.findById(params.id).populate("subcategories","_id category_id name description");
        return {success:true,subcategories:favorites && favorites.subcategories}     
      } 
