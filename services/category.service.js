...
async function getAll() {
  return  await Category.find({}, { isDeleted: false }).populate('subcategories');
}
      ...
      await Category.findOne({ name: catParam.name })
        .then(async category => {
          if (category) {
            ...
            req.files.icon&&(  category.icon = baseurl + req.files.icon[0].filename);
            req.files.banner&&( category.banner = baseurl + req.files.banner[0].filename);
            await category
              .save()
              .then(category => {
                resolve({
      ...
      await Category.findById(catParam.id)
        .then(async category => {      
          ...
          const { name,
            ...
            if(name){
            Category.findOne({name:name}).then(cat=>{
              if(cat)
              {
                resolve({success:false,
                  message :'An category with this name already exists. Please choose a different name...'});
              }
           ...
           sorting_options && ( category.sorting_options = JSON.parse(sorting_options));
