...
function addStates() {
  ...
  states.data.map((st) => {
    var state = new State();
    if (counter >= 0 && counter < 10) state._id = "ST00" + counter;
    else if (counter >= 10 && counter < 100) state._id = "ST0" + counter;
    else if (counter >= 100 && counter < 1000) state._id = "ST" + counter;
    // state.oldId=st.id;
    // state.name=st.name;
...
function addCities() {
  ...
  if (counter >= 0 && counter < 10) stateId = "ST00" + counter;
  else if (counter >= 10 && counter < 100) stateId = "ST0" + counter;
  ...
  // state.parentId=1001;
  // state.save();
  if (counter === 5) clearInterval(countDown);
  counter++;
}
  ...
  this.updateCountdown = function () {
    ...
    State.findOne({ _id: stateId })
      .then((st) => {
        ...
        request(
          "https://www.olx.in/api/locations?parent=" +
            st.oldId +
            "&hideAddressComponents=true",
          options,
          (err, response, body) => {
            if (err) {
              console.log(err);
    ...
    if (counter === 36) clearInterval(self.interval);
    counter++;
  };
